...
class GridworldEnv(EcoEnvironment):
    ...
    def __init__(
        ...
        if self.do_fruits:
            ...
            self.n_clusters = self.n_clusters_x_fruit_i * self.n_clusters_y_fruit_i
            self.n_quadri_clusters_x = self.n_clusters_x_fruit_i // 2  # 20 // 2 = 10
            self.n_quadri_clusters_y = self.n_clusters_y_fruit_i // 2
            self.n_quadri_clusters = self.n_quadri_clusters_x * self.n_quadri_clusters_y
            ...
            # Define the empty clusters map
            if self.prop_empty_clusters > 0:
                n_empty_clusters = int(
                    self.n_clusters_x_fruit_i * self.n_clusters_y_fruit_i
                    * self.prop_empty_clusters
                )
                print(f"[INFO] Number of empty clusters: {n_empty_clusters}")
                if n_empty_clusters > self.n_clusters:
                    raise ValueError("n_empty_clusters cannot be larger than the total number of clusters")
                # Create a flat array with k True and the rest False
                self.clusters_to_empty = np.array([True] * n_empty_clusters + [False] * (self.n_clusters - n_empty_clusters))
                np.random.shuffle(self.clusters_to_empty)
                # Reshape into a 2D map
                self.clusters_to_empty = self.clusters_to_empty.reshape((self.n_clusters_x_fruit_i, self.n_clusters_y_fruit_i))
                self.clusters_to_empty = jnp.array(self.clusters_to_empty)
    ...
    def step_grow_fruits(
        ...
        for coords_center, id_fruit in self.coords_clusters_to_fruit_id.items():
            ...
            # Extract the boolean corresponding to whether it is an empty cluster
            if self.prop_empty_clusters > 0:
                x = coords_center[0] // self.range_cluster_fruits
                y = coords_center[1] // self.range_cluster_fruits
                is_not_empty_cluster = 1 - self.clusters_to_empty[x, y]
            else:
                is_not_empty_cluster = 1
                    ...
                    for name_value_fruit in names_values_fruits_considered:
                        ...
                        for name_density_fruit in names_density_fruits_considered:
                            ...
                            for name_density_agents in names_density_agents_considered:
                                ...
                                # Old way : fully fill the vision field with agents and fruits randomly
                                # # Set the density of agents
                                # key_random, subkey = jax.random.split(key_random)
                                # visual_field = visual_field.at[:, :, :, idx_agent].add(jax.random.bernoulli(subkey, p=density_agents, shape=visual_field[:, :, :, idx_agent].shape))

                                # # Set the density of fruits
                                # key_random, subkey = jax.random.split(key_random)
                                # visual_field = visual_field.at[:, :, :, idx_fruit].add(jax.random.bernoulli(subkey, p=density_fruit, shape=visual_field[:, :, :, idx_fruit].shape))

                                # New way : create a pseudo-cluster of fruits and agents in the center of the visual field
                                key_random, subkey = jax.random.split(key_random)
                ...
                for name_density_fruit in names_density_fruits_considered:
                    ...
                    for name_density_agents in names_density_agents_considered:
                        ...
                        if "age" in self.list_observations:
                            obs["age"] = jnp.full(n, 50) / self.age_max
                        if "energy" in self.list_observations:
                            obs["energy"] = jnp.full(n, 50) / self.energy_max
                        if "n_childrens" in self.list_observations:
                            obs["n_childrens"] = jnp.zeros(n)
                        if "novelty_hunger" in self.list_observations:
                            obs["novelty_hunger"] = (
    ...
    def add_pseudo_cluster(
        ...
        )  # Boolean mask for easier selection
