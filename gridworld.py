...
AGENT_VIEW = 7
from agent import MetaRnnPolicy_bcppr
from agent import metaRNNPolicyState_bcppr
...
class Gridworld(VectorizedTask):
    ...
    def __init__(self,
        ...
        def reset_fn(key):
            ...
            pos_food_x = random.randint(next_key, (16 * self.nb_agents,), 1, (SX - 1))
            next_key, key = random.split(key)
            pos_food_y = random.randint(next_key, (16 * self.nb_agents,), 1, (SY - 1))
            next_key, key = random.split(key)
